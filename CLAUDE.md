...
- [x] OpenTelemetry: tracing em Django, Celery, Redis, Postgres, `requests` + `trace_id`/`span_id` no `OutboxEvent` linkando o request original à task assíncrona. Jaeger local via `docker-compose.yml`, **e também em produção** desde `docker-compose.prod.yml` (serviço `jaeger`, UI só em loopback — acesso via `ssh -L 16686:localhost:16686`). Guia: `docs/backend/observability.md`. Falta só a instrumentação do Next.js (frontend).
- [x] Feature flags por customer — tabela `FeatureFlag` própria (`apps/flags`), não Unleash: consistente com o resto do projeto (cada necessidade é um app Django, não um serviço de terceiro). Liga globalmente ou por customer específico (M2M), sem rollout por %. Resolvido em `/auth/me/` (`feature_flags: string[]`) e lido no front via `useAuthStore.hasFlag(key)`. CRUD em `/backoffice/feature-flags`.
- [ ] Terraform para camada de infra hoje coberta por `setup.sh` — **pausado**: a "VPS" é na verdade um HP EliteDesk físico numa rede local (192.168.1.x), e o host onde o Docker roda é ele mesmo uma LXC (`systemd-detect-virt` confirma), provavelmente sob Proxmox dado o padrão de LXCs separadas (túnel Cloudflare, monitoramento). Sem credenciais de API pra Proxmox/Cloudflare/Keycloak, não dá pra escrever Terraform testável — decisão explícita do usuário foi deixar parado até haver um motivo real (staging/DR) ou as credenciais serem disponibilizadas.
- [~] Medir p99 de `/validate-key` com dados reais de OTel antes de decidir sobre o serviço Go/gRPC (item avaliativo, não comprometido) — infra pronta (item acima), mas **ainda sem leitura real**: precisa de tráfego orgânico acumulando no Jaeger de produção antes de qualquer p99 fazer sentido. Consultar via `ssh -L 16686:localhost:16686 papermoon` → `http://localhost:16686`, serviço `papermoon-django`, operação do endpoint.
